...
resume_path = '/home/woody/rlvl/rlvl129v/data/models/control_sd15_canny.pth'
batch_size = 2
logger_freq = 30
learning_rate = 1e-5
sd_locked = True
...
#repeated_tensor = torch.stack([torch.tensor(item).repeat(1, 2, 1, 1) for item in state_dict['control_model.input_hint_block.0.weight']]).squeeze(1)
...
trainer = pl.Trainer(gpus=1, precision=32, callbacks=[logger], max_epochs = max_epochs, accumulate_grad_batches = accumulate_grad_batches)
